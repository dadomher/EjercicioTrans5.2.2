...
vec3 cPos, cTarget, cDirec, cRight, cUp, cFront;
//
...
void DoMovement(GLFWwindow* window) {
	...
	if (Status_W == GLFW_PRESS || Status_W == GLFW_REPEAT) cPos += cameraSpeed * cFront;
	else if (Status_S == GLFW_PRESS || Status_S == GLFW_REPEAT) cPos -= cameraSpeed * cFront;
	else if (Status_A == GLFW_PRESS || Status_A == GLFW_REPEAT) cPos -= glm::normalize(cross(cFront, cUp)) * cameraSpeed;
	else if (Status_D == GLFW_PRESS || Status_D == GLFW_REPEAT) cPos += glm::normalize(cross(cFront, cUp)) * cameraSpeed;
}
